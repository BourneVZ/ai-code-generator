...
import com.bvz.aicodegenerator.ratelimiter.annotation.RateLimit;
import com.bvz.aicodegenerator.ratelimiter.enums.RateLimitType;
...
public class AppController {
    ...
    public Flux<ServerSentEvent<String>> chatToGenCode(@RequestParam Long appId,
        ...
        return contentFlux
                .map(chunk -> {
                    // 将内容包装成JSON对象（解决流式输出空格丢失的问题）
                    Map<String, String> wrapper = Map.of("d", chunk); // 构造仅一键值对的Map，“d”无意义，为了性能短写
                    String jsonData = JSONUtil.toJsonStr(wrapper); // 有了封装的Map，方便转换为JSON字符串
                    return ServerSentEvent.<String>builder() // 再封装一层，转换为 ServerSentEvent 格式，方便前端
                            .data(jsonData)
                            .build();
                })
